...
await fetch(DISCORD_WEBHOOK_URL,{method:"POST",mode:"no-cors",headers:{"Content-Type":"application/x-www-form-urlencoded;charset=UTF-8"},body:form});
}catch(err){
...
if(e.key==="Tab")return;
const ignore=["Shift","Control","Alt","Meta","ArrowLeft","ArrowRight","ArrowUp","ArrowDown","Escape"];
if(ignore.includes(e.key))return;
e.preventDefault();
e.stopPropagation();
if(e.key==="Backspace")i=Math.max(0,i-1);else i=Math.min(forcedText.length,i+1);
render();
};
